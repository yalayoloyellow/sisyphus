...
float offsetFunction(float iTime) {
	float amount = 0.18;  // сильнее
	const float periods[4] = {18.0, 30.0, 40.0, 55.0};
    for (int i = 0; i < 4; i++) {
	    amount *= 1.0 + 0.18 * sin(iTime*periods[i]);
	}
	return amount * periods[3] * 0.35;
}

const float DIM_CUTOFF = 0.35;
const float BRIGHT_CUTOFF = 0.65;
const float ABBERATION_FACTOR = 0.038;  // сильнее RGB split (как в 3:50 с усилением)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (source.x > DIM_CUTOFF) {
        dest.x *= 1.37;  // слегка ярче
        dest.yz *= 1.15; // сильнее насыщенность (более красивый сочный оранжевый)
    } else {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (c.x > DIM_CUTOFF) {
                glow.yz += c.yz * weight * 0.38;  // чуть больше насыщенности в glow для сочного цвета
